...
import {BookingTicketAction, GetChairsAction} from "../../redux/actions/TicketAction";
...
export default function BookingTicketPage(props) {
    ...
    const renderChair = () => {
        return filmTicketData?.danhSachGhe?.map((chair, index) => {
            ...
            if (_.some(arrTicketChose, {maGhe: (chair.maGhe)})) chairClass = 'chair-booking';

            return <button key={index} className={chairClass} onClick={() => {
                let arrTicketUpdate = [...arrTicketChose];
                if (_.some(arrTicketUpdate, {maGhe: chair.maGhe})) {
                    arrTicketUpdate = arrTicketUpdate.filter(ghe => ghe.maGhe !== chair.maGhe)
        ...
        <div className="booking-ticket my-3">
            <div className="container mx-auto grid grid-cols-12 shadow-xl p-4">
                ...
                <div className="payment-detail col-span-12 md:col-span-3">
                    <h3 className="text-green-500 text-center text-3xl font-bold">{calSum()}đ</h3>
                    <hr className='my-2'/>
                    ...
                    <div className='grid grid-cols-2 text-lg'>
                        ...
                        <span className="text-right text-green-500 font-bold">{calSum()}đ</span>
                    </div>
                    <ul className='mt-2 px-1.5 ticketChose'>
                        {renderSelectedChair()}
                    </ul>
                    <hr className='my-2'/>
                    <ul className=''>
